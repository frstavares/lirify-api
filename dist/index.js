...
console.log(process);
app.listen(port, () => {
    console.log(`> Ready On Server http://${process.env.HOST}:${port}`);
});
const clientID = '6569af9a855a4837ba83e6518597de53';
const clientSecret = '2c76a66b557d4995900000ac32b66f1f';
const redirectUrl = `http://${process.env.HOST}:3000/`;
/**
...
passport.use(new SpotifyStrategy({
    clientID,
    clientSecret,
    callbackURL: `http://${process.env.HOST}:${port}/callback`,
...
app.get('/callback', passport.authenticate('spotify', { failureRedirect: '/login' }), (req, res) => {
    ...
    const user = req.user;
    res.redirect(`${redirectUrl}?accessToken=${user === null || user === void 0 ? void 0 : user.accessToken}&refreshToken=${user.refreshToken}&expiresIn=${user.expiresIn}`);
});
...
app.get('/lyrics', async (req, res) => {
    ...
    const searchResults = await node_fetch_1.default(`https://genius.com/api/search/multi?q=${encodeURI(query)}`);
    if (searchResults) {
        const geniusResponse = await searchResults.json();
        console.log(geniusResponse.response.sections);
        const section = geniusResponse.response.sections.find((section) => section.type === 'top_hit');
        if (!section) {
            res.status(404);
            res.send();
            return;
        ...
        const geniusLyricsResponse = await node_fetch_1.default(lyricsResult.result.url);
